-- =============================================================================
-- MariaDB/MySQL deployment copy, generated from ../00_Master_Tables.sql for the local
...
-- The design source (../00_Master_Tables.sql) remains the source of truth for schema
...
CREATE TABLE RM_MASTER (
    ...
    REPORTING_RM_CODE    VARCHAR(7),  -- self-ref: reporting manager
    DATE_OF_JOINING      DATE,
    EMPLOYMENT_STATUS    VARCHAR(20),           -- Active / Inactive
    CREATED_DATE         TIMESTAMP    NOT NULL DEFAULT CURRENT_TIMESTAMP,
    UPDATED_DATE         TIMESTAMP NULL DEFAULT NULL,
    SOURCE_SYSTEM        VARCHAR(30),
    CONSTRAINT FK_RM_MASTER_BRANCH_CODE FOREIGN KEY (BRANCH_CODE) REFERENCES BRANCH_MASTER(BRANCH_CODE),
    CONSTRAINT FK_RM_MASTER_REPORTING_RM_CODE FOREIGN KEY (REPORTING_RM_CODE) REFERENCES RM_MASTER(RM_CODE)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;
...
CREATE TABLE CLIENT_MASTER (
    ...
    UPDATED_DATE          TIMESTAMP NULL DEFAULT NULL,
    SOURCE_SYSTEM         VARCHAR(30),
    CONSTRAINT FK_CLIENT_MASTER_SECTOR_CODE FOREIGN KEY (SECTOR_CODE) REFERENCES INDUSTRY_SECTOR_MASTER(SECTOR_CODE),
    CONSTRAINT FK_CLIENT_MASTER_HOME_BRANCH_CODE FOREIGN KEY (HOME_BRANCH_CODE) REFERENCES BRANCH_MASTER(BRANCH_CODE)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;
...
CREATE TABLE CLIENT_RM_MAPPING (
    CLIENT_RM_MAPPING_ID  BIGINT       NOT NULL PRIMARY KEY AUTO_INCREMENT,
    ...
    UPDATED_DATE           TIMESTAMP NULL DEFAULT NULL,
    SOURCE_SYSTEM          VARCHAR(30),
    CONSTRAINT FK_CLIENT_RM_MAPPING_APR_CLIENT_CODE FOREIGN KEY (APR_CLIENT_CODE) REFERENCES CLIENT_MASTER(APR_CLIENT_CODE),
    CONSTRAINT FK_CLIENT_RM_MAPPING_RM_CODE FOREIGN KEY (RM_CODE) REFERENCES RM_MASTER(RM_CODE)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;
...
CREATE TABLE ACCOUNT_MASTER (
    ...
    UPDATED_DATE          TIMESTAMP NULL DEFAULT NULL,
    SOURCE_SYSTEM         VARCHAR(30),
    CONSTRAINT FK_ACCOUNT_MASTER_APR_CLIENT_CODE FOREIGN KEY (APR_CLIENT_CODE) REFERENCES CLIENT_MASTER(APR_CLIENT_CODE),
    CONSTRAINT FK_ACCOUNT_MASTER_PRODUCT_CODE FOREIGN KEY (PRODUCT_CODE) REFERENCES PRODUCT_MASTER(PRODUCT_CODE),
    CONSTRAINT FK_ACCOUNT_MASTER_BRANCH_CODE FOREIGN KEY (BRANCH_CODE) REFERENCES BRANCH_MASTER(BRANCH_CODE),
    CONSTRAINT FK_ACCOUNT_MASTER_CURRENCY_CODE FOREIGN KEY (CURRENCY_CODE) REFERENCES CURRENCY_MASTER(CURRENCY_CODE)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;
